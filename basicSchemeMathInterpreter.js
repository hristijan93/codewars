...
function scheme(cmd) {
    let formatted = cmd.replace(')', '').replace('(', '');
    let [operation, ...rest] = formatted.split(` `);
    ...
    switch(operation) {
        ...
        case '-': {
            return rest.reduce((acc, cur, index) => Number(index === 0 ? cur : acc - cur), 0)
        }
        case '*': {
            return rest.reduce((acc, cur) => acc * cur, 1);
        }
        case '/': {
            return rest.reduce((acc, cur, index) => Number(index === 0 ? cur : acc / cur), 1)
        }
    }
}
